...
def boxes_to_transformation_targets(ground_truth_boxes, anchors, weights=(10., 10., 5., 5.)):
    r"""
    Method to compute targets for each anchors or priors.
    ...
    target_dw = log(gt_w / anchor_w)
    target_dh = log(gt_h / anchor_h)
    :param ground_truth_boxes: (Tensor of shape N x 4)
    :param anchors: (Tensor of shape N x 4)
...
def apply_regression_pred_to_anchors(box_transform_pred,
                                     anchors,
                                     weights=(10., 10., 5., 5.)):
    r"""
    Method to transform anchors based on transformation parameter
    prediction.
    Assumes boxes are in x1y1x2y2 format
...
def generate_anchors(feat, aspect_ratios, scales):
    r"""
    Method to generate anchor boxes for all feature maps the image
    ...
    :return: anchors : List[(Tensor of shape N x 4)] anchors overall
            feature maps aggregated for each batch image
    """

    # List to store default anchor boxes for all feature maps
    default_boxes = []
    for k in range(len(feat)):
        ...
        # These shifts will be the centre of each of the anchor boxes
...
class SSD(nn.Module):
    ...
    6. Generate anchors for all these feature maps(8732 x 4)
    7a. If in training asign targets for these anchors and
    ...
    def __init__(self, config, num_classes=21):
        ...
        #self.obj_scales = [0.1, 0.2, 0.375, 0.55, 0.725, 0.9, 1.0]
        ...
        backbone = torchvision.models.vgg16(weights=torchvision.models.VGG16_Weights.IMAGENET1K_V1)
        ...
        for channels, aspect_ratio in zip(out_channels, self.aspect_ratios):
            ...
            self.cls_heads.append(nn.Conv2d(channels, self.num_classes * (len(aspect_ratio)+1),
                                            kernel_size=3,
                                            padding=1))
    ...
    def compute_loss(
        ...
        # Counting all the foreground anchors for computing N in loss equation
        num_foreground = 0
        # BBox losses for all batch images(for foreground anchors)
        bbox_loss = []
        # classification targets for all batch images(for ALL anchors)
        cls_targets = []
        for (
            targets_per_image,
            ...
            # Foreground anchors -> matched_idx >=0
            # Background anchors -> matched_idx = -1
            foreground_idxs_per_image = torch.where(matched_idxs_per_image >= 0)[0]
            foreground_matched_idxs_per_image = matched_idxs_per_image[foreground_idxs_per_image]
            num_foreground += foreground_matched_idxs_per_image.numel()

            # Get foreground anchors and their transformation predictions
            matched_gt_boxes_per_image = targets_per_image["boxes"][foreground_matched_idxs_per_image]
            bbox_regression_per_image = bbox_regression_per_image[foreground_idxs_per_image, :]
            anchors_per_image = anchors_per_image[foreground_idxs_per_image, :]

            # From foreground anchors and their assigned gt boxes,
            # compute transformation targets
            target_regression = boxes_to_transformation_targets(matched_gt_boxes_per_image, anchors_per_image)

            bbox_loss.append(
                torch.nn.functional.smooth_l1_loss(bbox_regression_per_image,
                                                   target_regression, reduction='sum')
            )

            # Get classification target for ALL anchors
            # For all anchors set it as 0 first
            # Then set foreground anchors target as label
            # of assigned gt box
        ...
        # for all batch images for all foreground anchors
        bbox_loss = torch.stack(bbox_loss)
        cls_targets = torch.stack(cls_targets)

        # Calculate classification loss for ALL anchors
        num_classes = cls_logits.size(-1)
        cls_loss = torch.nn.functional.cross_entropy(cls_logits.view(-1, num_classes), cls_targets.view(-1), reduction="none").view(
            cls_targets.size()
        )

        # Hard Negative Mining
        foreground_idxs = cls_targets > 0
        # We will sample total of 3 x (number of foreground anchors) background anchors
        num_negative = self.neg_pos_ratio * foreground_idxs.sum(1, keepdim=True)

        # As of now cls_loss is for ALL anchors
        negative_loss = cls_loss.clone()
        # We want to ensure that after sorting based on loss value,
        # foreground anchors are never picked when choosing topK
    ...
    def forward(self, x, targets=None):
        ...
        for i, features in enumerate(outputs):
            ...
            # Permute cls output from (B, A * num_classes, H, W) to (B, HWA, num_classes).
            N, _, H, W = cls_feat_i.shape
            cls_feat_i = cls_feat_i.view(N, -1, self.num_classes, H, W)  # (B, A, num_classes, H, W)
            cls_feat_i = cls_feat_i.permute(0, 3, 4, 1, 2)  # (B, H, W, A, num_classes)
            cls_feat_i = cls_feat_i.reshape(N, -1, self.num_classes)  # (B, HWA, num_classes)
        ...
        anchors = generate_anchors(outputs, self.aspect_ratios, self.scales)
        # anchors -> List[Tensor of shape 8732 x 4]
        ...
        if self.training:
            ...
            # List to hold for each image, which anchor box
            ...
            for anchors_per_image, targets_per_image in zip(anchors, targets):
                if targets_per_image["boxes"].numel() == 0:
                    ...
                    continue
                iou_matrix = get_iou(targets_per_image["boxes"], anchors_per_image)
                # For each anchor box find best ground truth box
                matched_vals, matches = iou_matrix.max(dim=0)

                # Update index of match for all anchors which
                ...
                # We want to also assign the best anchor box for every gt
                # as foreground
                # So first find the best anchor box for every gt
                _, highest_quality_pred_foreach_gt = iou_matrix.max(dim=1)
                # Update the best matching gt index for these best anchors
                # as 0, 1, 2, ...., len(gt)-1
            ...
            losses = self.compute_loss(targets, cls_logits, bbox_reg_deltas, anchors, matched_idxs)
        else:
            # For test time we do the following:
            # 1. Convert anchors to boxes using predicted bbox regression deltas
            # 2. Low score filtering
            # 3. Pre-NMS filtering
            # 4. NMS
            # 5. Post-NMS filtering
            cls_scores = torch.nn.functional.softmax(cls_logits, dim=-1)
            num_classes = cls_scores.size(-1)

            detections = []

            for bbox_deltas_i, cls_scores_i, anchors_i in zip(bbox_reg_deltas, cls_scores, anchors):
                boxes = apply_regression_pred_to_anchors(bbox_deltas_i, anchors_i)
                ...
                keep_indices = torch.ops.torchvision.nms(pred_boxes, pred_scores, 0.45)
